...
module.exports = {
    name: "update",
    description: "Send updates to customer",
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const attatchment = await new MessageAttachment(img.toBuffer(), "Update.png");
            ...
            interaction.followUp({ content: "Update send"});
            await interaction.channel.send({ content: `<@${data.customerId}> Here is the update for today`, embeds: [SucessEmbed] });
            await interaction.channel.send({ files: [attatchment] });
            await service.updateOne({ channelId: channelID }, { experience: stats["skills"]["overall"]["xp"] })
        } catch (error) {
            console.log(error)
        };
    }
